...
# ---- id maps ----
# Player renames applied on top of the workbook (which still carries the old
# value). Keeps a regen consistent with the hand-corrected JSON data. The id
# remap flows through PLY into EVERY reference (roster, scores, matches, drafts,
# awards, champion, captains); the name remap only touches the display name.
SLUG_RENAME = {"scott-b": "scott-benesh"}
NAME_RENAME = {"Scott B": "Scott Benesh"}
def norm_slug(s): return SLUG_RENAME.get(s, s)
def norm_name(n): return NAME_RENAME.get(n, n)

PLY = {}  # PLY-00x -> slug
for r in rows("DB Players"):
    PLY[r["Player ID"]] = norm_slug(r["Profile Slug"])
...
players = []
for r in rows("DB Players"):
    pid = norm_slug(r["Profile Slug"])
    prev = _existing.get(pid, {})
    players.append({
        "id": pid,
        "name": norm_name(r["Player Name"]),
        "slug": pid,
...
SUMMARY = {
 ...
 3: {"summary": "Ed Nelson and Scott Benesh took control from the opening hole and never allowed the Woodpeckers into the match. They were 1 Up after the 1st, 2 Up through two and 3 Up after the 3rd. After a halved 4th, the Silver Spoons won the 5th and 6th to reach 5 Up. Colton McKivitz and Chase Hellmers managed to win the 7th and briefly reduce the margin, but the response was short-lived. Ed and Scott restored the five-hole advantage at the 9th to complete the most one-sided result of the opening session. The match was effectively decided by the Silver Spoons' five-hole winning burst across the first six holes.",
     "standout": "Scott Benesh — he helped set the pace from the first hole and anchored the dominant 5&3 victory."},
 ...
 5: {"summary": "This was one of the most even matches of the tournament. The sides were all square through the 8th before Michael Herring and Rupert Pedler edged 1 Up at the 9th and carried that lead to the 10th. Steve Urwin and Scott Benesh pulled the match level at the 11th and moved 1 Up at the 12th. The Woodpeckers answered at the 13th, but the Silver Spoons again moved ahead at the 14th and remained 1 Up through the 15th. Michael and Rupert squared it at the 16th, and neither team could win either of the final two holes. The lead changed hands repeatedly, with both teams producing an answer whenever the other appeared ready to take control.",
...
moments = [
    ...
    {"id": "st-george-2026-moment-4", "tournamentId": "st-george-2026", "roundNumber": 4,
     ...
     "description": "Tom Brunskill paired Chase Hellmers and Rupert Pedler against Alan Lozer and Scott Benesh to squeeze every handicap stroke out of the Stableford round."},
