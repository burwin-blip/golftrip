...
import { submitRsvp, readRsvp, RsvpError } from '../../lib/rsvp-api.js';
...
function fail(e) {
  ...
  return json({ ok: false, error: 'Something went wrong saving that. Please try again.' }, 500);
}

export async function GET({ url }) {
  try { return json(await readRsvp(url.searchParams.get('player'))); } catch (e) { return fail(e); }
}

export async function POST({ request }) {
  let body;
  try { body = await request.json(); } catch { return json({ ok: false, error: 'Bad request.' }, 400); }
  try { return json(await submitRsvp(body)); } catch (e) { return fail(e); }
}
